...
export function IntegrationWorkflow({ listingId }: { listingId?: string }) {
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      ...
      <div className="bg-background border border-border rounded-xl p-6 flex flex-col gap-4">
        <div className="flex items-center gap-3">
          ...
          <h3 className="font-bold text-lg">CRM Property Site</h3>
        </div>
        <p className="text-sm text-muted-foreground flex-1">
          Automatically create or update the RealEstateCRM property site with the newly generated high-res images and 16:9 walkthrough video.
        </p>
        <div className="bg-muted/20 border border-border rounded-md p-3 text-xs text-muted-foreground space-y-1">
          <div className="flex justify-between"><span>Status:</span> <span className="font-semibold text-green-500">Connected</span></div>
          <div className="flex justify-between"><span>Target:</span> <span>123-main-st.realestatecrm.com</span></div>
        </div>
