#!/usr/bin/env node
/**
 * MyPlus "Neighborhood Data" → Lofty importer
 * ─────────────────────────────────────────────────────────────────────────────
 * Imports a saved MyPlus Neighborhood list (circle-prospecting leads) into
 * Lofty CRM the same way the Expired/FSBO MyPlus leads were imported, but with
 * only two hashtags:
 *    1. CIRCLE PROSPECT
 *    2. ESTATES LANE - MACOMB TWP
 *
 * Source data is the raw MyPlus `api/neighborhood/search` response saved to
 * data/neighborhood-estates-lane-source.json (list: "ESTATES LANE 1-Macomb Twp",
 * listId 1197648, filterId 911019).
 *
 * Every phone + email is collected from:
 *   - premiumDetails.contact1..6 (phone1..4, email1..3)
 *   - basicContact1 / basicContact2 (phone + email1/email2)
 *   - customPhones
 *
 * Usage:
 *   node scripts/LOFTY/import-neighborhood-leads.mjs --dry-run   # preview only
 *   node scripts/LOFTY/import-neighborhood-leads.mjs --commit    # create leads
 */
...
// scripts/LOFTY -> project root (2 levels up)
const ROOT = resolve(__dirname, '..', '..');
const ENV_PATH = resolve(ROOT, '.env.local');
const DATA_DIR = resolve(ROOT, 'data');
const SOURCE_PATH = resolve(DATA_DIR, 'neighborhood-estates-lane-source.json');
...
const LIST_NAME = 'ESTATES LANE 1-Macomb Twp';
...
function parseName(prop) {
  ...
  // Prefer the natural-person owner name (proper case) when available.
  ...
  // Trusts / LLCs / entities: use the premium contact person (title-cased).
  const pd = prop.premiumDetails || {};
  ...
  // Last resort: split the owner/entity name or fall back to the street.
  if (oName) {
    const tokens = oName.split(/\s+/).filter(Boolean);
    return {
      firstName: trunc(tokens[0], 30),
...
function buildNote(prop, name) {
  ...
  for (let i = 0; i < premiumContacts(prop).length; i++) {
    const c = premiumContacts(prop)[i];
...
async function main() {
  ...
  if (!existsSync(SOURCE_PATH)) {
    console.error(`❌ Missing source data: ${SOURCE_PATH}`);
    process.exit(1);
  }

  const raw = JSON.parse(readFileSync(SOURCE_PATH, 'utf8'));
  const properties = raw?.data?.properties || [];
  ...
  writeFileSync(
    resolve(DATA_DIR, 'neighborhood-estates-lane-built-payloads.json'),
    JSON.stringify(
      built.map((b) => ({
  ...
  writeFileSync(
    resolve(DATA_DIR, 'neighborhood-estates-lane-import-results.json'),
    JSON.stringify({ dryRun: DRY_RUN, listName: LIST_NAME, tags: TAGS, results }, null, 2),
  );
  console.log('\n📄 Results written to data/neighborhood-estates-lane-import-results.json');
}
